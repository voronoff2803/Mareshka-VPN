...
public class APIClient {
    ...
    public static var `default` = APIClient(baseURL: MatreshkaAPI.Server.main)
    ...
    public init(baseURL: String, sessionManager: Session = .default, defaultHeaders: [String: String] = [:], behaviours: [RequestBehaviour] = []) {
        ...
        jsonEncoder.dateEncodingStrategy = .formatted(MatreshkaAPI.dateEncodingFormatter)
    }
    ...
    private func makeNetworkRequest<T>(request: APIRequest<T>, urlRequest: URLRequest, requestBehaviour: RequestBehaviourGroup, completionQueue: DispatchQueue, complete: @escaping (APIResponse<T>) -> Void) -> Request {
        ...
        if request.service.isUpload {
            ...
            return sessionManager.request(urlRequest, interceptor: self)
                .validate().responseData(queue: decodingQueue) { dataResponse in
                    self.handleResponse(request: request, requestBehaviour: requestBehaviour, dataResponse: dataResponse, completionQueue: completionQueue, complete: complete)
        ...
        case .success(let value):
            do {
                ...
                if let error = error as? DecodingError {
                    ...
                    if error.description == "Unexpected status code: 403" && MatreshkaHelper.shared.accessToken != "" {
                        MatreshkaHelper.shared.showAlert(message: "Access Token error: \(MatreshkaHelper.shared.accessToken)", error: true)
                    }
                    apiError = error
                } else {
...
extension APIClient: RequestInterceptor {
    public func adapt(_ urlRequest: URLRequest, for session: Session, completion: @escaping (Result<URLRequest, Error>) -> Void) {
        ...
        if request.url?.absoluteString.contains("refresh") == true {
            completion(.success(request))
            return
        }
        
        let access_token = MatreshkaHelper.shared.accessToken
    ...
    public func retry(_ request: Request, for session: Session, dueTo error: Error,
        ...
        case 200...299:
            completion(.doNotRetry)
        case 403:
            if MatreshkaHelper.shared.refreshToken != "" {
            ...
            break
        case 400:
            if let urlStriing = request.request?.url?.absoluteString {
                if urlStriing.contains("refresh") {
    ...
    func refreshToken(completion: @escaping (_ isSuccess: Bool) -> Void) {
        APIClient.default.makeRequest(MatreshkaAPI.AuthAPI.RefreshToken.Request(body: RefreshTokenRequest(refreshToken: MatreshkaHelper.shared.refreshToken))) { response in
            switch response.result {
            case .success(let result):
                if let accessToken = result.success?.token {
                    MatreshkaHelper.shared.accessToken = accessToken
                    completion(true)
                } else {
                    completion(false)
                }
